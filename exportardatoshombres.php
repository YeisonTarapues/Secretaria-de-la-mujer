<?php
	...
	header('Content-Disposition: attachment; filename=Registros hombres.xls');
		...
		class Conexion{
			...
			function conectarse(){
				...
				if($enlace){
					echo "Registros de hombres";	//si la conexion fue exitosa nos muestra este mensaje como prueba, despues lo puedes poner comentarios de nuevo: //
				}else{
...
<table border="1">
	<tr style="background-color:#73A3FC;">
		<th>Idregistrohombres</th>
		<th>Municipio</th>
		<th>Mes</th>
		<th>Año</th>
	...
	<?php
		while ($row=mysqli_fetch_assoc($result)) {
			?>
				<tr>
					<td><?php echo $row['idregistrohombres']; ?></td>
					<td><?php echo $row['municipio']; ?></td>
					<td><?php echo $row['mes']; ?></td>
					<td><?php echo $row['anno']; ?></td>
					<td><?php echo $row['numero_hombres']; ?></td>
